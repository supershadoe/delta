...
internal fun LoadingButton() {
  ...
  val startAnimState =
    animateFloatAsState(targetValue = 1f, label = "HotspotButtonLoadingAnim")
  ...
  val shape =
    if (startAnimState.value == 1f) {
      RotatingShape(polygon = roundedPillStar, degrees = rotationState.value)
    } else {
      MorphingShape(
        morph = morphStarToPillStar,
